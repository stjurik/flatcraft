 ...
 * слово у повідомленні («сторону»/«діаметр») — за hole_shape. Перевіряємо обидві
 * форми, обидві осі, межу торкання та еталонний дефект каталожної панелі.
 */
import { describe, expect, it } from "vitest";

import { validatePerforation } from "./perforation.js";

describe("validatePerforation — square holes", () => {
  const base = {
    length_mm: 200,
    width_mm: 150,
    thickness_mm: 1.5,
...
describe("validatePerforation — circle holes", () => {
  const base = {
    length_mm: 200,
    width_mm: 150,
    thickness_mm: 1.5,
  ...
  it("обидва кроки менші за діаметр → дві HOLES_OVERLAP, message згадує діаметр", () => {
    const issues = validatePerforation({
      templateSlug: "perforated_panel",
      parameters: { ...base, hole_size_mm: 25, pitch_x_mm: 12, pitch_y_mm: 12 },
    });
    expect(issues).toHaveLength(2);
