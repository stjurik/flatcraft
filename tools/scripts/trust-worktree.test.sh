...
#
# ЧОМУ ЦЕЙ НАБІР ІСНУЄ. Він перевіряє семантику САМОГО СКРИПТА, а не поведінку
# `agy` — це різні речі, і плутанина між ними вже коштувала одного хибного
# висновку. Скрипт реєструє РІВНО корінь git переданої теки й вважає теку
# зареєстрованою лише за точним збігом; §7 фіксує саме це рішення (запис батька
# не рахується за реєстрацію дитини), бо від нього залежить ідемпотентність
# `add`/`remove` — інакше `remove` не знайшов би того, що додав `add`.
#
# Чого набір НЕ доводить: що `agy` поводиться так само. Виміри 2026-09-14 і
# 2026-09-15 суперечать одне одному (перший — браузерний OAuth з підтеки
# довіреного батька, другий — повний PASS з теки, якої в списку немає взагалі).
# Обидві точки в `docs/19` §D.4; цей файл їх не арбітрує.
set -uo pipefail
...
# ── 7. батьківський запис НЕ рахується за реєстрацію (семантика скрипта) ───
# Це твердження про `check`, не про `agy`: `add`/`remove`/`check` мусять бачити
# однаковий ключ, інакше `remove` не знайде того, що додав `add`.
