...
run_suite() {
  ...
  EXPECT_RE='жоден кандидат не знайдено'
  check "недовірена тека → міряє далі (0) + примітка у виводі" 0 pass no
  unset EXPECT_RE
  # Позитивна гілка: зонд мусить НАЗВАТИ знайденого кандидата. Доданий
  # 2026-09-16 разом із виправленням `--show-toplevel`: «поза списком» без
  # уточнення шляху — половина твердження, і саме вона брехала у worktree.
  EXPECT_RE='знайдено: workspace'
  check "довірена тека → зонд називає, ЯКИЙ кандидат знайдено" 0 pass yes
  unset EXPECT_RE
