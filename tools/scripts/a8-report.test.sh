...
run_report
missing=()
for s in "Таймери" "Чи запускався тік" "Журнал прогонів" "Запобіжники" "Черга" \
  "Worktree і диск" "Egress" "Креденшал" "Потрібне перезавантаження"; do
  grep -q "$s" "$OUT" || missing+=("$s")
done
((${#missing[@]} == 0)) && ok "знімок містить усі дев'ять розділів" ||
