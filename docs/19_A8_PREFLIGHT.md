...
>
> **Третя точка — 2026-09-16, ГІПОТЕЗА, не вимір `agy`.** Вимір М-1 (PR #110, сім
> прогонів) встановив для **Claude Code**: ключ довіри — не cwd і не тека
> worktree, а **корінь головного клону** (git-резолвінг, як `--git-common-dir`).
> Запис для теки worktree не діє взагалі.
>
> Якби `agy` резолвив проєкт так само, це пояснило б вимір 2026-09-14: зонд тоді
> запускався з `~/hart-wt/...`, а в списку стояв `~/hart-wt` — тобто **жоден** із
> двох кандидатів (ні сам worktree, ні корінь клону `~/hart`) у списку не був.
>
> **Це пояснення не підтверджене.** `agy` — інструмент іншого постачальника, і
> переносити на нього поведінку Claude Code без виміру не можна. Спроба виміряти
> 2026-09-16 (контрольний прогін: у списку лише стороння тека, cwd —
> `~/hart-wt/a8-stage-0`) дала **`PASS`, exit 0, nonce дослівно, без сигнатури
> браузерного логіна**. Тобто список не гейтить нічого, а за такої конструкції
> питання ключа **не має відповіді в принципі** — це не «відповідь негативна», а
> неможливість розділити варіанти. Прогони з окремими ключами тому не робились.
>
> **Переможця досі немає, і це навмисно.** Три точки лишаються поруч: браузерний
> OAuth 09-14, повний `PASS` без списку 09-15 і 09-16, гіпотеза про корінь клону.
>
> **Що з цього зроблено.** `tools/scripts/trust-worktree.sh`
> (`check`/`add`/`remove`, ідемпотентний) + виклик `add` в `autorun.sh` одразу
> після `worktree add` — **страховка, не передумова**: нічого не блокує, `exit 2`
> (немає `settings.json`) прогін не валить. Реєструються **обидва** кандидати —
> і тека worktree, і корінь клону, — бо резолвінг `agy` не виміряний, а обидва
> шляхи однаково правдоподібні. Коли резолвінг виміряють — лишити один.
>
> Заразом виправлено зонд: `check-agy-headless.sh` звіряв лише
> `--show-toplevel` і тому в worktree друкував «тека поза `trustedWorkspaces`»
> навіть тоді, коли корінь клону зареєстровано. Діагностика, здатна сказати
> неправду, — той самий клас, що знятий вердикт 7; тепер зонд звіряє обидва
> шляхи і називає, який саме знайдено. **Примітка зонда — це звірка шляху, а не
> поведінка `agy`:** вердикт виміру береться лише з exit-коду, nonce і наявності
> запиту логіна.
