...
| 7    | ⚠️ тека поза `trustedWorkspaces`                         | `trust-worktree.sh add <тека>` (застереження нижче) |
...
> ⚠️ **`trustedWorkspaces` — межа працездатності автономних прогонів, не лише зонда.**
> `agy` працює тільки в теках зі списку `trustedWorkspaces`
> (`~/.gemini/antigravity-cli/settings.json`); там сьогодні `~/hart` і
> `~/lun_monitor`. `autorun.sh` створює worktree у `$HOME/hart-wt/<...>`
> (`tools/scripts/autorun.sh`, `WT_ROOT`), а ADR-039 §2 будує ізоляцію саме на
> «власному worktree на задачу». Перевірено 2026-09-13: запуск зонда з
> `~/hart-wt/...` дає **exit 7** — тобто `agy` і worktree-ізоляція виглядали
> взаємовиключними.
>
> **Закрито виміром 2026-09-14.** Обидва варіанти опитування PR #108 («додати
> батьківську теку» / «тримати worktree всередині `~/hart`») спиралися на
> префіксне успадкування довіри, якого немає:
>
...
>
> `agy` звіряє **точний** корінь git, без успадкування вниз — тому й
> `~/hart/.worktrees/` не врятував би: піддеки мають власні корені. Отже
> реєстрація мусить бути **поточною**, на створення worktree, а не разовою
> правкою конфігу: це робить `tools/scripts/trust-worktree.sh`
> (`check`/`add`/`remove`, ідемпотентний, реєструє корінь git навіть коли
> передали підтеку), і `autorun.sh` кличе `add` одразу після `worktree add`.
> Ненастроєний `agy` (немає `settings.json` → exit 2) прогін не валить —
> лишається попередження про відсутній Gemini-пул.
