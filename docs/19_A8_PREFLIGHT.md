...
### D.4. `agy` / Gemini на headless-машині — НЕПЕРЕВІРЕНО

Логін `agy` на T470 йшов через браузер. На сервері без GUI цей шлях не описаний, і я
не знаю, який із варіантів спрацює. **Не вигадуйте — перевірте по черзі:**

1. Запустити `agy` по SSH і подивитись, чи дає він URL для авторизації на іншій машині.
2. Якщо ні — скопіювати каталог конфігу з T470 (`~/.gemini/` разом із
   `antigravity-cli/settings.json`) і перевірити, чи приймається сесія на іншому хості.
3. Якщо ні — API-ключ з aistudio.google.com у змінну оточення `agent`.

Що спрацювало — напишіть; від цього залежить, чи зможе демон використовувати
Gemini-пул як резерв, коли Claude-квота вичерпана. Заразом врахуйте, що `write_file(*)`
у `~/.gemini/antigravity-cli/settings.json` ширший за задум — звуження до
`docs/promts/inputs/` є в черзі «Відкладено до A8».
...
- [ ] D.4 — з'ясувати робочий шлях логіна `agy` на headless (3 варіанти, §D.4) —
      результат впливає на резервний квотний пул у ADR-039
