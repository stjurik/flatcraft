...
| 7    | ⚠️ тека поза `trustedWorkspaces`                         | запустити з довіреної теки (див. застереження нижче) |
...
> `~/hart-wt/...` дає **exit 7** — `agy` з автономного прогону недоступний у
> принципі. Рішення (розширити список / інша тека / інший шлях ізоляції) —
> за yurii, воно у «Опитуванні» PR'а, а не в цьому документі.
