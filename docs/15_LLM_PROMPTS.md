...
> Ролі — ADR-036 §3. Історичні «рівні» A/B/C збережено як позначки промптів нижче
> (A→Архітектор, B→Будівельник, C→Розвідник); Рев'юер — нова роль без власних
> промптів у цьому файлі (його промпт вбудований у `ai-review` workflow,
> `docs/promts/master-ai-bugfix-flow.md`).

| Роль (рівень)       | Тип завдання                                                                                                                                                                                       | Модель                                                                                                                                                                                                                                                                                                                                                                                                  | Доступ до репо                                                                                                        |
| ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| **Архітектор (A)**  | ADR, master-runs, архітектурні рішення, trade-off-аналіз, docs-gate PR, тріаж, huddle                                                                                                              | Найсильніша reasoning-модель (Claude Opus/Fable у Claude Code)                                                                                                                                                                                                                                                                                                                                          | Повний                                                                                                                |
| **Будівельник (B)** | Код за затвердженим ADR: чіткий scope, TDD, відомі паттерни                                                                                                                                        | Середня (Claude Sonnet у Claude Code)                                                                                                                                                                                                                                                                                                                                                                   | Повний                                                                                                                |
| **Рев'юер**         | Незалежне рев'ю AI-PR: diff проти плану й інваріантів §7/§13 → вердикт ✅/⚠️/❌                                                                                                                    | Два шляхи (ADR-036 §3, `CLAUDE.md` §0 п.2): основний — `agy`: основна рецензія **Gemini 3.8 Flash (High)**, друга, коротка, лише контрприклади — **Claude Opus 4.6**; кожна модель складає свій іспит (перші 10 викликів, межа 50%). Окрема сесія Claude (не та, що писала код) теж допустима, зокрема для архітектурних і безпекових PR, — і тоді в PR прямо пишеться, що рецензію зроблено цим шляхом | Read-only (diff/repo); через `agy` — запис лише в `docs/promts/inputs/`; ніколи не мержить                            |
| **Розвідник (agy)** | Read-only сканування/інвентаризація файлів (Паттерн 2, `docs/promts/agy-orchestration-recommendations.md`); headless `agy -p`, вивід ЛИШЕ у `docs/promts/inputs/`; кожен вихід — verify-then-write | Gemini 3.1 Pro (High), CLI `agy` (локальний агент). **Активний з 2026-07-20** (Master Run 8, Стадія 1); з 2026-09-23 — лише Розвідник: іспит Рев'юера не складено (`docs/20` OQ-35)                                                                                                                                                                                                                     | Read-only (repo й worktree-и); запис лише в `docs/promts/inputs/`, жодної команди — власні дозволи agy (`tools/agy/`) |
| **Розвідник (C)**   | Інвентаризація, аудит, чернетки текстів, підсумовування digest'ів                                                                                                                                  | Дешева/безкоштовна (Haiku, чат-моделі)                                                                                                                                                                                                                                                                                                                                                                  | **Read-only або без доступу** — результат у чат/markdown, ніколи не пише у репо                                       |
...
Порядок запуску визначають **черги треків** у `docs/02_ROADMAP.md` (ADR-036). Історична послідовність (C2 → A1 → B1/B2/B5 observability ∥ C1 → A2 registry — виконано; B3 — виконано; B4 → трек T2; A3 → трек T4) збережена у git-історії цього рядка.
