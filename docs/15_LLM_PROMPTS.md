...
| **Рев'юер**         | Незалежне рев'ю AI-PR: diff проти плану й інваріантів §7/§13 → вердикт ✅/⚠️/❌                                                                                                                    | Сильна Claude-модель в **окремій сесії** (не та, що писала код). **Gemini CLI (`agy`) — активний (2026-07-20, Master Run 8)**: перший headless-запуск як Розвідник підтвердив доступ; роль Рев'юера — Стадія 3 цього run'у (перше застосування) | Read-only (diff/repo); ніколи не мержить                                         |
| **Розвідник (agy)** | Read-only сканування/інвентаризація файлів (Паттерн 2, `docs/promts/agy-orchestration-recommendations.md`); headless `agy -p`, вивід ЛИШЕ у `docs/promts/inputs/`; кожен вихід — verify-then-write | Gemini 3.1 Pro (High), CLI `agy` (локальний агент). **Активний з 2026-07-20** (Master Run 8, Стадія 1)                                                                                                                                          | Read-only (repo); write обмежений на рівні agy-конфіга до `docs/promts/inputs/*` |
