...
| **Рев'юер**         | Незалежне рев'ю AI-PR: diff проти плану й інваріантів §7/§13 → вердикт ✅/⚠️/❌       | ЗАРАЗ — сильна Claude-модель в **окремій сесії** (не та, що писала код). Gemini CLI — умовна опція, `pending`: yurii ще не перевірив CLI-доступ; тригер — ADR-036 §3 | Read-only (diff/repo); ніколи не мержить                                        |
