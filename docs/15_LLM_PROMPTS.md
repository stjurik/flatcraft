...
| **Рев'юер**         | Незалежне рев'ю AI-PR: diff проти плану й інваріантів §7/§13 → вердикт ✅/⚠️/❌                                                                                                                    | Два шляхи (ADR-036 §3, `CLAUDE.md` §0 п.2): основний — `agy`: основна рецензія **Gemini 3.8 Flash (High)**, друга, коротка, лише контрприклади — **Claude Opus 4.6**; кожна модель складає свій іспит (перші 10 викликів, межа 50%). Окрема сесія Claude (не та, що писала код) теж допустима, зокрема для архітектурних і безпекових PR, — і тоді в PR прямо пишеться, що рецензію зроблено цим шляхом | Read-only (diff/repo); ніколи не мержить                                                                              |
