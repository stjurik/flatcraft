...
2. **Реалізацію перевіряє не автор.** ШІ-код рев'юїть інший агент (ADR-036:
   ...
   _Автономний режим:_ `agy`/Gemini — **read-only пул** (ADR-039 §5). Підстава
   кількісна: `docs/promts/inputs/agy-stats.md` — 3 виклики з 3 містять ≥1
   вигаданий факт. Його вихід — джерело гіпотез, ніколи не джерело фактів без
   верифікації Claude або yurii.
...
**Гейти:** merge — yurii. **Єдиний виняток** — auto-merge оборотного класу у
тестовому режимі (ADR-040): усі оракули зелені + вердикт рев'юера ✅ + diff
поза переліком виключень. Виняток скасовується автоматично в момент публічного
launch. Обмеження §6 (міграції, залежності, `infra/`, `.github/`, `CLAUDE.md`,
ADR, bend-матриця, снапшоти) діють **завжди** — включно з автономними run'ами і
включно з режимом auto-merge.
...
⚠️ **Фактичний стан захисту (перевірено 2026-08-10, `docs/16` §1):** трекований
`.claude/settings.json` містить лише `permissions.allow`; ключа `deny` немає.
Механічних обмежень у цьому режимі **сьогодні нуль** — усе тримається на
`--allowedTools`, тексті промпту і на тому, що yurii поруч. До перенесення
портативних deny-правил у git (ADR-039, крок 5 треку T5) не залишай локальний
автономний прогін без нагляду.
...
5. **Незалежне рев'ю** — інший агент, не той, що писав код (ADR-036 §3).
   Вердикт публікується дослівно. `agy`/Gemini — read-only пул з обов'язковою
   верифікацією (ADR-039 §5).
6. **Приймання yurii — очима, на прев'ю.** Draft PR + URL прев'ю-середовища +
   розділ «Як перевірити очима» → канал `#на-перевірку`.
7. **Merge — yurii.** Виняток: auto-merge оборотного класу в тестовому режимі
   (ADR-040) — усі оракули зелені + вердикт ✅ + diff поза виключеннями; після
   merge staging → smoke → авто-revert при червоному; добовий дайджест
   змердженого. Скасовується автоматично на публічний launch.
...
- **Промпти середовища:** `docs/promts/master-inspection-contract.md` (Master Run 14), `docs/promts/master-a8-transition.md` (розгортання середовища — файл ще не створено), результат інспекції — `docs/promts/inputs/inspection-2026-08.md`
