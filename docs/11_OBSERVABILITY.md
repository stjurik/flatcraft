...
Umami (self-hosted) + Discord webhook. Жодного Prometheus/Grafana/OTel/Kafka/K8s.
...
| 2. Продуктова аналітика | де користувач страждає       | Umami (воронка) + web-vitals             | R-10                  |
...
Продуктова аналітика — **Umami self-hosted** (ADR-032 §4): окрема БД у наявному Postgres-контейнері,
cookie-less (ADR-006 → без cookie-banner). Plausible відхилено (CE потребує ClickHouse ~2 ГБ RAM;
Cloud = кошти + дані поза Україною).
...
- Web-vitals (LCP/CLS/INP/FCP/TTFB) — custom-подія `web_vital`; `rating` звіряється з бюджетами
  CLAUDE.md §9.
- **Реалізація (PR 5):** клієнт-обгортка `apps/web/src/lib/analytics.ts` (vendor-нейтральний
  `track` — no-op на SSR / без завантаженого скрипта; **fail-closed на PII** у props). Umami-скрипт
  вантажиться `AnalyticsScripts` лише коли задано `NEXT_PUBLIC_UMAMI_WEBSITE_ID` +
  `NEXT_PUBLIC_UMAMI_SRC`; web-vitals — `WebVitals` (`useReportWebVitals`). Врізка кроків:
  `catalog` (CatalogToggle) · `studio_opened`/`param_changed`/`validation_error_shown`
  (TemplateStudio) · `export_clicked`/`export_done` (ExportButton). Нуль нових залежностей.
- Події до ініціалізації Umami-скрипта дропаються (немає queue-stub) — accepted при поточному
  трафіку (10 users/day; втрата хвоста ранніх подій незначна).
- **Umami vs `events` — не взаємозамінні:** Umami — агрегатна воронка/UX-сигнал (клієнт-side,
  семпльований); `events` — точний серверний факт (для digest, аудиту, join з `exports`). Один
  констатує намір користувача, другий — результат на сервері.

> **Стан деплою Umami:** контейнер, окрема БД у Postgres і Caddy-vhost `analytics.hart.crimea.ua` —
> у `infra/compose/docker-compose.prod.yml`, `infra/compose/Caddyfile`, `infra/ansible/roles/flatcraft/`
> (Phase 3.3-B, `docs/08 §7`). Активація в проді — за 7-крок runbook'ом (створення Sentry-проєктів,
> Discord webhook, UMAMI_APP_SECRET, DNS, Umami website). Тригер перегляду стеку аналітики: понад
> 500K подій/міс (ADR-032 §4).
