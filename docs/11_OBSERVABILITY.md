...
| `cad_started`         | worker                 | старт CAD-операції                   | template_slug                          |
| `cad_completed`       | worker                 | кінець CAD-операції                  | template_slug, duration_ms             |
...
- **worker (Python):** `cad_started`, `cad_completed` (з `duration_ms`) — через наявний
  worker→api-callback, **не** окремий публічний роут (див. `docs/06_API_CONTRACT.md`,
  внутрішній контракт).
...
| PR   | Зміст                                                        | Статус |
| ---- | ------------------------------------------------------------ | ------ |
| PR 1 | ADR-032 + цей файл + Roadmap + preview Data Model/API (docs) | цей PR |
