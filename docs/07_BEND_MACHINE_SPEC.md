...
**Інваріант напряму згину:** напрям (UP/DOWN) — **обов'язковий атрибут кожного гибу** (виробничі креслення вимагають його, інакше виробник може зробити дзеркальну деталь). Дефолт — `down` для всіх шаблонів (рішення замовника). Рендериться на креслі: стрілкою (↓/↑) у колонці «Напрям» bend-table PDF і поряд з callout на розгортці; ASCII-міткою `DOWN/UP` у DXF bend-text. Напрям не впливає на геометрію розгортки (довжину/позиції гибів) — лише на анотацію. Деталі — ADR-019, CLAUDE.md §7.
