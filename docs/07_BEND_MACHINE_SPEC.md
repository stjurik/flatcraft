...
**Інваріант сервера (Hotfix 2.10.e, ADR-019):** ця валідація — обов'язкова **на сервері**, не лише у браузері. `cad-engine validateBend` викликається у Fastify-gate `POST /exports` ПЕРЕД forward у воркер; Python-воркер дублює перевірку матриці на старті `/export` (defense-in-depth). Клієнтська валідація — лише UX-підсвічення і тривіально обходиться. Паритет TS+Python покрито property-based тестами (1000 ітерацій) проти цього ж YAML.

**Інваріант напряму згину:** напрям (UP/DOWN) — **обов'язковий атрибут кожного гибу** (виробничі креслення вимагають його, інакше виробник може зробити дзеркальну деталь). Дефолт — `down` для шаблонів з одним гибом; для **Z-кронштейна дефолт `[down, up]`** (профіль фізично гнеться у протилежні боки). Рендериться на креслі **текстовою позначкою `UP`/`DOWN`** — у колонці «Напрям» bend-table PDF, поряд з callout на розгортці (`BEND #n DOWN R… d=…`) і у DXF bend-text (`BEND 90° DOWN R… #n`). Напрям не впливає на геометрію розгортки (довжину/позиції гибів) — лише на анотацію. Деталі — ADR-019, CLAUDE.md §7.
...
## 7. DXF Specification (виробничий формат — ADR-024)

Експортований DXF призначений для прямого завантаження у CAM-софт виробництв лазерного різання (Lantek, SigmaNest, ESI native). Формат **навмисно мінімальний**: лише геометрія різання і гибу, жодного тексту/розмірів (це CAM-noise і колись був причиною P0-бага — CAM пропускав отвори на окремому шарі → деталь без отворів).

**Конвенція двох шарів:**

| Шар          | ACI color | Linetype     | Що містить                                                                                                 |
| ------------ | --------- | ------------ | ---------------------------------------------------------------------------------------------------------- |
| `LASER_CUT`  | 7 (white) | `CONTINUOUS` | **ВСІ** cut-paths: зовнішній контур (`LWPOLYLINE`, ByLayer) + отвори (`CIRCLE`, ByEntity **color 5 blue**) |
| `BEND_LINES` | 3 (green) | `DASHED`     | лінії гибу (`LINE`) — info для оператора, **не ріжуться**                                                  |

**Правила color coding:**

- Зовнішній периметр — **ByLayer** (не має власного кольору, успадковує 7 від `LASER_CUT`).
- Внутрішні вирізи (отвори) — той самий шар `LASER_CUT`, але **explicit ByEntity color 5 (blue)**. Так CAM і людина візуально відрізняють inner-cut від outer, не розносячи їх по різних шарах.
- `BEND_LINES` — окремий шар, color 3 + dashed, щоб CAM відфільтрував їх з cut-операції.

**Чого у DXF НЕМАЄ (на відміну від PDF):** TEXT-анотацій, DIMENSION-розмірів, Ø-виносок, напряму гибу (UP/DOWN), номерів гибів. Усе це — у супровідному PDF, для людини.

**Службові шари:** `0` і `Defpoints` створює сам ezdxf і їх неможливо видалити (DXF-інваріант). Вони лишаються **порожніми** — жодного entity. «Рівно 2 шари» стосується виробничих; службові ігноруються.

**Як перевірити файл (LibreCAD / AutoCAD / FreeCAD):**

1. Відкрити DXF → у панелі шарів має бути `LASER_CUT` (білий) і `BEND_LINES` (зелений пунктир), плюс порожні `0`/`Defpoints`.
2. Зовнішній контур і всі отвори — на `LASER_CUT`; отвори візуально сині (color 5).
3. Жодного тексту/розміру на кресленні.
4. Програмно (ezdxf): `doc.modelspace().query("CIRCLE")` — усі на `LASER_CUT` з `color == 5`; `query("LWPOLYLINE")` — рівно один, ByLayer (color 256); `query("TEXT") + query("DIMENSION")` — порожньо.

Регресія зафіксована у `workers/cad/tests/test_dxf_export.py` (structural-snapshot на 5 шаблонів + integration reopen).
