...
### T5. Інфраструктура → автономне середовище розробки (вісім кроків)

**Мета:** перейти від «людина запускає run'и вручну» до «оркестратор веде затверджений
план, yurii задає напрямок і приймає результат очима». Джерело послідовності — план
yurii від 2026-08-07; рішення Q1–Q13 — `docs/19_A8_PREFLIGHT.md` §0.2; контракт
середовища — **ADR-039**, visibility — **ADR-038**, auto-merge — **ADR-040**, критерії
prod-переїзду — **ADR-041 (Proposed)**.

**Правило треку:** критерій виходу з кожного кроку — **число або зелений чекпоінт**,
не відчуття «вже працює». Крок без виконаного критерію не вважається закритим, навіть
якщо наступний уже почався.

**Черга (вісім кроків):**

1. ✅ **Хвости — жодної роботи, що існує лише на диску.** Закрито: PR **#99**
   (артефакти, що жили лише локально), PR **#98** (правдива черга T1), PR **#101**
   (`z_bracket` на Registry). _Критерій виходу:_ `git status` у `~/hart` чистий на
   недільному ритмі — виконано (перевірено 2026-08-11: порожній вивід).
2. **Сервер A8 — мануальний preflight** (`docs/19_A8_PREFLIGHT.md`, PR **#103**).
   Треки A (Ubuntu 24.04, BIOS «Restore on AC Power Loss»), B (користувачі без sudo,
   SSH key-only, UFW, swap), C (Docker, Node 22, pnpm, uv, Playwright), D (deploy key →
   private → runner → токени → тунель → Discord). _Критерій виходу:_ **Checkpoints A, B,
   C, D зелені** (команди перевірки — у самому документі), усі пункти чеклиста в
   issue #78 викреслені. **Блокує кроки 5–8.**
3. ✅ **Інспекція + контракт — Master Run 14.** Стадія 1 — PR **#104**
   (`docs/promts/inputs/inspection-2026-08.md`, таблиці A–G з доказами). Стадія 2 —
   цей PR: ADR-038, ADR-039, ADR-040, ADR-041 + `docs/promts/inputs/claude-md-2.0.md`.
   _Критерій виходу:_ чотири ADR змерджено; текст CLAUDE.md 2.0 застосовано yurii
   (`cp`) або свідомо відхилено з причиною.
4. **Генеральний план — окремий run** (не частина Master Run 14, свідомо).
   Вхід — змерджена інспекція; вихід — беклог задач, де **кожна задача має названий
   оракул приймання** (тест/скрипт/снапшот), а не формулювання «зробити добре».
   Місце беклогу — GitHub issues з лейблами (Опитування Master Run 14, п.2).

   > **Оракул мусить бути СИЛЬНИМ, а не просто бути.** Перевірка зараховується,
   > лише якщо вона червоніє на **неправильному** результаті, а не тільки на
   > відсутньому. Куплено першим справжнім прогоном демона (2026-09-20, задача
   > `log-pr116`): оракул вимагав, щоб у файлі був рядок `#116` і щоб `prettier`
   > був чистий — обидві умови виконуються при будь-якому вмісті абзацу. Агент
   > написав абзац із вигаданим числом (18 пар «вимога→assertion» замість 24
   > реальних), оракул був зелений, і помилку впіймало око.
   > Практичний критерій при постановці задачі: **назви, що саме зламається в
   > оракулі, якщо агент зробить роботу неправильно.** Немає відповіді — оракул
   > заслабкий, і задачу в такому вигляді брати не можна.

   _Критерій виходу:_ **≥ 20 задач** заведено, у **100 %** з них заповнене поле
   «оракул приймання», і **≥ 1 задача кожного з трьох джерел** (напрямок yurii /
   аналітика / фідбек).

5. **Розгортання середовища — master-run `docs/promts/master-a8-transition.md`** (промпт ще не написаний — пишеться на початку цього кроку)
   (`docs/19` §6): Ansible-роль `a8`, контейнер агента, демон-супервізор з чергою,
   класифікатор виходу, kill switch, денний ліміт, журнал, прев'ю-стек через
   Cloudflare Tunnel, портативні deny-правила у трекованому `.claude/settings.json`,
   перенос важких CI-job на self-hosted. _Критерій виходу:_ демон піднімає **тестову
   задачу end-to-end** (черга → worktree → сесія → draft PR) **3 рази поспіль без
   ручного втручання**; kill switch зупиняє чергу **≤ 60 с**; машина після примусового
   ребуту повертається у робочий стан **≤ 5 хв** без участі людини.
6. **Етап 1 — автономія під наглядом.** Агент виконує задачі беклогу, кожен PR читає і
   мерджить yurii (ADR-036 §5 без змін). _Критерії виходу (усі, у ковзному вікні
   14 днів):_
   - **≥ 10** задач доведено до draft PR без втручання в процес виконання;
   - **≥ 8 з 10** змерджено без ручних доробок yurii поверх;
   - **0** випадків запису у виключений шлях (перевірка `check-forbidden-paths.sh` на
     кожному PR);
   - **≥ 80 %** задач мають програмний доказ приймання (метрика «покриття оракулами»,
     ADR-039 §8);
   - **≤ 3** питання класу A на 10 задач і **0** питань, відхилених супервізором як
     невалідні (незаповнені обов'язкові поля);
   - kill switch перевірено **≥ 1 разу** навчальною зупинкою.
7. **Етап 2 — auto-merge оборотного класу** (ADR-040, лише поки платформа в тестовому
   режимі). _Критерії виходу (усі, у ковзному вікні 30 днів):_
   - **≥ 20** PR пройшли auto-merge;
   - **≤ 1** auto-revert по червоному smoke на 20 merge (**≤ 5 %**);
   - **0** auto-merge, що зачепив виключений шлях (ADR-040 §1);
   - **0** інцидентів на staging, що вимагали ручного втручання довше **30 хв**;
   - добовий дайджест змердженого опубліковано у **≥ 90 %** діб.
8. **Етап 3 — самокерований беклог.** Сигнал (аналітика, виробничий фідбек, Sentry)
   перетворюється на задачу без участі yurii; yurii лишається на напрямку і прийманні.
   Передумова — закриті розриви з інспекції §E (digest-cron існує в git; шлях
   «сигнал → issue» автоматизовано хоча б для одного джерела). _Критерій виходу:_ його
   немає — **вихід із Етапу 3 — це публічний launch (трек T3)**, який автоматично
   скасовує auto-merge (ADR-040 §4) і повертає ADR-036 §5 у повному обсязі.

**Окремо (не крок, а вимірювання, що йде паралельно з 6–8):** ADR-041 — квартал
збору uptime-даних A8 за шістьма метриками з фіксованими наперед порогами. Рішення про
переїзд prod з Mirohost MS21 **не приймається** до кінця кварталу; чинним лишається
ADR-011.

**Гейт yurii:** merge кожного ADR і кожного master-run'а; мануальні кроки — issue #78;
перехід між кроками 5→6→7 — явне рішення yurii за критеріями виходу, а не
автоматичний.
...
1. **`git status` ПЕРЕД digest'ом** — переконатись, що жодна робота не існує лише на
   ...
   **Те саме для машини A8:** `cd infra/ansible && ansible-playbook a8.yml -i inventory.a8.ini --check --diff`.
   Будь-який `changed` означає, що машина розійшлась із роллю: або руками змінене те, чим
   роль керує, або роль не застосована. Знайдене — або задача в роль, або issue.
   **Межа детектора:** він бачить лише те, чим роль керує. Ручний файл, про який роль не
   знає (так на A8 знайдено `docker-prune.service/.timer` і
   `docker.service.d/ufw-reload.conf`, 2026-09-18), `--check` не покаже ніколи — такі
   знахідки йдуть окремим issue. Нової машинерії не пишемо: ритуал, що працює, дешевший
   за таймер, який ніхто не читає.
