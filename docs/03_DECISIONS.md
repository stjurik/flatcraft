...
| ADR-036 | Модель розробки 2.0 — треки, ритми, мультиагентний конвеєр                         | Accepted; §3 переглянуто 2026-09-24 — OQ-35                     | 2026-07-18          |
| ADR-037 | i18n-архітектура — власні словники + `/en/*` routing (Etap A)                      | Accepted                                                        | 2026-07-20          |
| ADR-038 | Repo visibility → private (MIT лишається на роздані знімки)                        | Accepted; виконання (D.1) відкладено 2026-09-17 — OQ-30         | 2026-08-11          |
| ADR-039 | Автономне середовище розробки на A8 — оракули в CI, агенти на залізі               | Accepted                                                        | 2026-08-11          |
| ADR-040 | Auto-merge оборотного класу у тестовому режимі                                     | Accepted (скасовується на публічний launch)                     | 2026-08-11          |
| ADR-041 | Критерії переїзду prod з Mirohost MS21 — метрики і пороги                          | **Proposed** (рішення не приймається)                           | 2026-08-11          |
...
**Примітка (model-routing, acceptance-run 2026-07-19):** `ai-triage.yml` спочатку йшов на
`--model opus` — четвертий acceptance-прогін (issue #84) впав миттєвим `is_error:true` ($0,
0 turns) одразу після успішного system-init з резолвнутим `"model": "claude-opus-4-8"`.
Квоту виключено (скріншот yurii: Session 62%, Weekly 24% на момент падіння) — робоча гіпотеза:
`CLAUDE_CODE_OAUTH_TOKEN` (Claude Pro-план) не має entitlement на `claude-opus-4-8` через
OAuth-обмін у headless CI-контексті. Модель тріажу змінено на `--model sonnet` (той самий
alias, що вже стабільно працює в `ai-fix.yml`): доступний на Pro-плані, тріаж (класифікація +
коментар + лейбл) не вимагає топ-моделі, дешевша квота. `ai-fix.yml` без змін — уже був на
`sonnet`.

**Фактичний результат (2026-08-03, Master Run 10+11).** Acceptance пройдено наскрізь:
issue #84 → тріаж у CI (прогін `30616535686`) → `ai-approved` від yurii → Будівельник у CI
(прогін `30700893565`, draft PR #97) → незалежне рев'ю `agy`/Gemini + верифікація Claude →
merge (`d4c81be`) → перевірка очима на staging. Рішення цього ADR лишаються чинними без
ревізії — зокрема рішення 3 (двигун Claude, рев'ю локальне мультиагентне).

Але **гіпотеза примітки вище — спростована**. Зміна моделі тріажу фіксом НЕ була: справжня
причина п'яти падінь — **недійсне ЗНАЧЕННЯ секрету `CLAUDE_CODE_OAUTH_TOKEN`**. Перший
прогін після перевипуску токена (`claude setup-token` → `gh secret set`) пройшов зеленим
без жодної додаткової зміни. Діагностичний доказ став видимим лише тоді, коли тимчасово
ввімкнули `show_full_output` (PR #86) і в логу з'явився дослівний текст:
`Failed to authenticate. API Error: 401 Invalid bearer token`, `"api_error_status": 401`,
`num_turns: 1`, `total_cost_usd: 0`.

Дорогою перевірено й відкинуто **чотири гіпотези** — саме вони коштували тижня простою
треку T1, тому фіксуються поіменно:

1. **`max-turns`/невірна модель/tool-permissions** (знахідка №1-2, 2026-07-19) — спростовано
   читанням логів: system-init проходив успішно.
2. **Вичерпана квота підписки** (знахідка №3) — спростовано скріншотом yurii (Session 62%,
   Weekly 24% на момент падіння).
3. **Pro-план не має entitlement на `claude-opus-4-8` у headless CI** (знахідка №4, PR #86) —
   спростовано: після переходу на `sonnet` падіння повторилось із тією ж сигнатурою.
4. **Застаріла версія `claude-code-action`** (PR #94, v1.0.178 → v1.0.183) — спростовано:
   на новій версії падіння повторилось.

Спільна сигнатура всіх чотирьох хибних слідів — «миттєвий `is_error`, $0, 0-1 turn одразу
після успішного system-init». Тепер вона задокументована в `docs/17` §9 як підпис 401-auth,
а не моделі. Урок процесу: дії з `show_full_output: false` за замовчуванням ховають дослівний
текст помилки SDK — без нього діагностика вироджується у перебір гіпотез.
...
## ADR-038: Repo visibility → private (MIT лишається на роздані знімки)

**Статус:** Accepted (2026-08-11, Master Run 14 Стадія 2). Виконання — крок D.1
`docs/19_A8_PREFLIGHT.md` (мануальний, issue #78); до його виконання репозиторій
фактично `PUBLIC` (`gh repo view` на 2026-08-11: `{"visibility":"PUBLIC"}`).

**Контекст:** Рішення Q1 (`docs/19` §0.2), джерело — аналіз 2026-07-14 і черга T5 п.1
(`docs/02_ROADMAP.md`). Мета — **не пускати комерційне використання майбутніх
напрацювань**: платформа неприбуткова, без юрособи, і поки код відкритий, будь-хто
може підняти платний клон швидше, ніж автор доведе свій до launch.

До цього рішення додався **технічний блокер, який сам по собі достатній**: self-hosted
runner на публічному репозиторії означає, що PR із чужого форку виконує довільний код на
домашній машині yurii (`docs/19` §0.3, жорстка залежність «runner тільки після private»).
Без private неможливий саме **self-hosted runner** (крок D.2 `docs/19`) — і лише він.
Решта треку A8 від visibility не залежить: ADR-039 у власній назві розділяє «оракули в
CI, агенти на залізі», а агент на A8 раннером не є — чужих PR він не виконує і в CI не
входить.

> **Виправлено 2026-09-17** за рішенням yurii «зараз лише мультиагентне середовище на
> A8» (`docs/20_OPEN_DECISIONS.md`, OQ-30). Попереднє формулювання — «Без private весь
> трек A8 (ADR-039) неможливий у безпечному вигляді» — узагальнювало на весь трек
> аргумент, справедливий лише для D.2. Воно спростоване власною роботою проєкту:
> **Стадія 0 і крок 0 Стадії 1 виконані на публічному репозиторії** (2026-09-15…17,
> PR #109 і #110). Доки той рядок стояв, наступний читач ADR-038 робив висновок, що
> Стадія 1 заблокована до переводу репозиторію в private — тобто зупинявся перед
> роботою, яка вже виконується.

Поточний стан, перевірений: `LICENSE` (MIT) у корені; `README.md:6,15,137,157`; UI-рядки
`apps/web/src/i18n/dictionaries.ts:214,242` (uk) і `:390,418` (en) — бейдж «Open Source ·
MIT» у trust-row і картка «Open Source MIT»; принцип CLAUDE.md §2.6 «Open Source MIT з
першого коміту».

**Рішення:** перевести `stjurik/flatcraft` у **private**. MIT-ліцензія на вже роздані
знімки НЕ відкликається (юридично неможливо) — файл `LICENSE` лишається в репозиторії
для всіх комітів, зроблених до перемикання. Приватність захищає **майбутнє**, не минуле.

**Альтернатива (розглянута і відхилена): лишити public і змінити ліцензію на
AGPL-3.0 / PolyForm Noncommercial.**

- _Чому відхилено (три причини, кожна самостійно достатня):_
  1. **Не досягає мети.** Зміна ліцензії так само не відкликає MIT на вже роздані
     знімки — той самий обмежувач, що й у private-варіанта. Різниця лише в тому, що
     public+copyleft лишає весь МАЙБУТНІЙ код видимим, тобто мету «не пускати
     комерційне використання напрацювань» він досягає загрозою позову, а не механікою.
  2. **Немає кому виконувати загрозу.** Enforcement copyleft-ліцензії соло-розробником
     без юрособи (CLAUDE.md §1) — теорія. Ліцензія, яку ніхто не може захистити в суді,
     на практиці дорівнює MIT з довшим текстом.
  3. **Не знімає технічний блокер.** Self-hosted runner на public-репо лишається
     виконанням чужого коду на домашній машині незалежно від ліцензії (`docs/19` §0.3).
- _Що з цього лишається чинним:_ якщо після публічного launch з'явиться потреба
  повернути відкритість (contributors, довіра), правильний хід — re-open ПІД
  явно обраною ліцензією, окремим ADR, а не відкат цього рішення «мовчки».

**Наслідки:**

- **(−) R-07 (`docs/04_RISKS.md:144`) слабшає.** Mitigation 1 «Open Source MIT →
  community може підхопити» перестає діяти для нового коду: якщо yurii вибуває,
  приватний репозиторій ніхто не підхопить. **Компенсація (обов'язкова, входить у
  scope A8-run'у, `docs/19` §6):** щоденний `git clone --mirror` репозиторію + копія
  pg_dump на A8, зберігання поза єдиним хостом. Це знижує ризик «проєкт зник разом з
  акаунтом», але не відновлює «спільнота підхопить» — це свідома плата за мету.
- **(−) GitHub Actions перестають бути безлімітними.** На публічному репо хвилини на
  стандартних раннерах не тарифікуються; на приватному діє місячна квота плану.
  **Виміряна вартість (інспекція §G, 3 останні PR):** #103 → 18 billed-хв, #101 → 16,
  #99 → 15; середнє **16.3 billed-хв/PR** (wall-clock 10.0 хв/PR). Конкретна квота
  акаунта **НЕ ПЕРЕВІРЕНА**: `gh api /users/stjurik/settings/billing/actions` повертає
  `404` (потрібен OAuth-scope `user`), `gh api user --jq .plan.name` → `null`. За
  документованими тарифами GitHub це 2000 хв/міс (Free) або 3000 хв/міс (Pro), що дає
  **≈122 або ≈184 повних CI-прогони на місяць** при виміряних 16.3 хв. Навіть нижня
  оцінка кратно перевищує поточний темп (4 змерджені PR за 2026-08-05…08-11).
  → Мануальний крок yurii: подивитись план і фактичну квоту в Settings → Billing
  (issue #78). Число з промпту цього run'у («ліміт 2000 хв/міс») у репозиторії
  **не підтверджене** — див. розділ «Розбіжності» у PR.
- **(−) Оцінка 25–40 хв/PR, що фігурувала як обґрунтування self-hosted раннера,
  спростована виміром** (16.3). Economics-аргумент «переносимо CI, бо хвилин не
  вистачить» слабкий; чинним лишається інший аргумент ADR-039 §1 — перенос **важких**
  job (Q2) заради швидкості й e2e-стабільності, а не заради квоти.
- **(+) Технічний блокер знято:** після D.1 можна реєструвати runner (D.2).
- **(=) Deploy не ламається.** `infra/ansible/group_vars/all.yml` використовує SSH
  (`git@github.com:…`), read-only deploy key вже створений (`docs/08` §1.5), deploy
  keys працюють на private однаково; образи тягнуться автентифіковано (`docker login
ghcr.io` під `vault_ghcr_token`). Факт зафіксовано у `docs/19` §D.0 — **перевірка D.0
  блокує D.1** і лишається мануальним кроком, а не припущенням.
- **Правки, які тягне це рішення** (виконуються ОКРЕМИМИ run'ами, не цим):
  1. `README.md` (рядки 6, 15, 137, 157) — бейдж MIT, «open source (MIT)», секція
     «Ласкаво просимо PR-и», посилання на LICENSE: переформулювати як «MIT до
     `<дата перемикання>`; репозиторій приватний, зовнішні PR не приймаються».
  2. `apps/web/src/i18n/dictionaries.ts` — `trustOpenSource` (uk :214 / en :390) і
     картка «Open Source MIT» (uk :242 / en :418): trust-row не має стверджувати
     неправду про поточний стан. Це **код**, тому поза scope docs-run'у.
  3. `CLAUDE.md` §2.6 «Open Source MIT з першого коміту» — **цей run НЕ змінює**:
     §2 «Принципи розробки» заморожено умовою «явної згоди yurii» цього run'у
     (byte-identical). Зміна принципу — рішення yurii окремим кроком; занесено в
     issue #78. Формально до цієї правки §2.6 і ADR-038 суперечать один одному, і
     джерелом істини є ADR (пізніше рішення), а не §2.6.
  4. `docs/04_RISKS.md` R-07 — mitigation 1 переписати на бекап-компенсацію.

**Альтернативи:** «private з подальшим re-open після launch» (варіант 3 з T5 п.1) —
не відхилено, а **відкладено**: воно не суперечить цьому ADR і приймається окремо після
launch, коли буде видно, чи є з ким ділити код. «Лишити public і не робити A8» —
відхилено: це відмова від усього треку автономної розробки, для якого і робиться
Master Run 14.

---

## ADR-039: Автономне середовище розробки на A8 — оракули в CI, агенти на залізі

**Статус:** Accepted (2026-08-11, Master Run 14 Стадія 2). Реалізація — окремий
master-run (`docs/promts/master-a8-transition.md` — **ще не написаний**, `docs/19` §6);
цей ADR фіксує контракт середовища, не його код.

**Контекст:** Цільова модель, сформульована yurii 2026-08-07: «Ми розгортаємо
середовище автономної розробки. Потужний агент-оркестратор веде реалізацію
затвердженого плану, дешевші агенти виконують задачі. Вхід для розвитку — три джерела:
напрямок, який задає yurii; аналітика платформи; зворотний зв'язок користувачів і
виробництва. yurii відповідає на концептуальні питання і приймає готовий функціонал
очима.»

Вхідні дані — інспекція `docs/promts/inputs/inspection-2026-08.md` (Стадія 1, PR #104).
Чотири її висновки прямо формують це рішення:

1. **Клас розриву «код + тест, 0 продакшн-викликів»** (§A): `validateHoles` (issue #78,
   2026-07-21), `validateSheet` (`packages/cad-engine/src/validators/sheet.ts:20,39-46`,
   0 викликів поза власним тестом), §7 п.5 «CadQuery `isValid`» — механізму немає в
   кодовій базі взагалі. Зелений тест у цих випадках **не доводить enforcement**.
2. **Локальний headless-прогін не має жодного механічного deny** (§C.2):
   `.claude/settings.json` (трекований, 37 записів) містить лише `permissions.allow`;
   ключа `deny` немає. Увесь реальний захист живе в `.github/workflows/ai-fix.yml:62-81`
   (19 записів `deny` = 8 файлових цілей парою `Edit`+`Write` + 3 Bash-патерни) + `tools/scripts/check-forbidden-paths.sh:19-26` (8 патернів).
3. **`agy`/Gemini систематично вигадує деталі**: `docs/promts/inputs/agy-stats.md` —
   3 записи, у кожному ≥1 «вигаданий факт» (2, 1, 1), жодного разу 0.
4. **Історія п'яти падінь тріажу** (`docs/13_PROGRESS_LOG.md:13`, прогони
   `29695180231`, `29695927414`, `30558791848`, `30563391403`, `30612966665`):
   недетермінований агент може падати мовчки і зелено, а чотири правдоподібні гіпотези
   з'їдають тиждень. Детермінований крок такого класу відмов не має.

Рішення Q1–Q13 (`docs/19` §0.2) — вхід, не предмет перегляду.

**Рішення** (вісім складових):

**1. Лінія розділу: GitHub Actions = детерміновані оракули, A8 = недетерміновані агенти.**

- _Альтернативи:_ усе в Actions (статус-кво ADR-035); усе на A8 (включно з CI).
- _Вибір:_ **детермінований крок ніколи не виконує агент, недетермінований крок ніколи
  не є гейтом.** У GitHub Actions лишаються 10 job'ів `ci.yml`, `deploy-staging.yml`,
  `release.yml` — все, що має відтворюваний вихід. На A8 живуть агенти (планування,
  імплементація, рев'ю) — усе, що дає різний вихід на однаковому вході.
- _Обґрунтування:_ інспекція §B показує, що кожен оракул має точну межу
  («доводить / НЕ доводить»); межу можна перевірити тільки якщо оракул детермінований.
  Історія §4 контексту — ціна змішування: агент у CI падав так, що job був зеленим
  (`30616535686` — перший зелений тріаж узагалі), а стан issue не змінювався.
- _Наслідки:_ (+) при червоному CI завжди зрозуміло, що саме зламано. (−) два
  середовища замість одного; перенос важких job на self-hosted (Q2) — окремий PR.

**2. Ізоляція замість підтверджень.**

- _Альтернативи:_ інтерактивні підтвердження (не працюють без людини); allowlist без
  ізоляції (статус-кво `docs/16` §2 — інспекція §C.2 довела, що він порожній).
- _Вибір:_ агент працює **у контейнері**, під користувачем `agent` (без sudo, `docs/19`
  §B.1), у **власному git-worktree на задачу**, з **allowlist вихідної мережі**.
  `--dangerously-skip-permissions` дозволений **ЛИШЕ всередині контейнера** (Q10) і
  заборонений на хості й на робочій машині yurii.
- _Обґрунтування:_ підтвердження — механізм для людини поруч; коли її немає, вони або
  блокують роботу, або їх знімають цілком. Ізоляція обмежує **наслідки** помилки замість
  того, щоб намагатись її не допустити. Worktree на задачу — уже перевірений патерн
  (`docs/16` §4) і пряма реакція на 2 інциденти самовільного запису `agy` (Master Run 8).
- _Інваріант (не побажання — умова існування середовища), джерело `docs/19` §7:_
  **на A8 НЕМАЄ пароля Ansible-vault, НЕМАЄ SSH-ключа до Mirohost, НЕМАЄ GHCR-токена на
  запис.** Тому `deploy-staging.yml` і `release.yml` **лишаються на GitHub-hosted**
  раннерах назавжди в межах цього ADR. Агент не деплоїть — деплоїть CI після merge.
  Порушення цього рядка = компрометація домашньої машини дорівнює компрометації prod.
- _Наслідки:_ (+) найгірший наслідок збою агента — зіпсований worktree, не зіпсований
  prod. (−) контейнер треба збирати й підтримувати; частина задач (перевірка деплою)
  агенту недоступна принципово.

**3. Оркестратор — тонкий детермінований супервізор, стан у git і черзі на диску.**

- _Альтернативи:_ довга сесія-«менеджер», що тримає план у контексті; Actions-джоба як
  двигун (відхилено ще на Q6).
- _Вибір (Q13):_ демон-супервізор без LLM усередині: бере задачу з черги на диску,
  створює worktree, запускає **свіжу сесію на задачу**, класифікує вихід (складова 4),
  пише журнал, повторює. **Стан ніколи не живе в контексті моделі** — лише в git
  (гілки, PR) і файлах черги.
- _Обґрунтування:_ довга сесія коштує дорого (>150k контексту на кожен наступний крок)
  і втрачає весь стан при падінні; свіжа сесія на задачу має фіксовану вартість входу і
  відновлюється перезапуском. Детермінований супервізор сам є оракулом (складова 1).
- _Наслідки:_ (+) падіння агента = втрата однієї задачі, не всього прогону; вартість
  прогнозована. (−) задачі мають бути сформульовані так, щоб виконуватись без пам'яті
  про попередні — це вимога до беклогу (складова 8, «покриття оракулами»).

**4. Класифікатор виходу — три класи, три реакції.**

| Клас                | Сигнатура (джерело)                                                                                                                                                                                                     | Реакція                                                                                                                                                |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Автентифікація**  | `Failed to authenticate. API Error: 401 Invalid bearer token`, `"api_error_status": 401`, `num_turns: 1`, `total_cost_usd: 0`, миттєвий `is_error` (`docs/03` ADR-035, `docs/17` §9, `docs/13:13` — 5 прогонів 2026-07) | **STOP усієї черги** + повідомлення в `#інциденти`. Потрібен інтерактивний логін (`claude setup-token`) — автоматика тут неможлива за визначенням.     |
| **Квота**           | **СИГНАТУРИ НЕМАЄ.** Гіпотезу «вичерпана квота» перевірено і спростовано 2026-07-30 (скріншот yurii: Session 62%, Weekly 24%) — тобто реального лог-прикладу вичерпання у проєкті **не спостерігалось жодного разу**    | **Пауза до наступного вікна** + запис у `#інциденти`. Детектор пишеться за фактом першої появи; до того цей клас **порожній** — див. «Наслідки» нижче. |
| **Інше (реальний)** | усе, що не підпало під два попередні                                                                                                                                                                                    | задача → `failed`, повний журнал на диск, наступна задача з черги; три `failed` поспіль → пауза черги.                                                 |

- _Обґрунтування і головне застереження:_ 401 має **дослівну** сигнатуру з реальних
  логів — її можна матчити рядком. Квотної сигнатури в проєкті не існує, і **вигадати
  її було б рівно тією помилкою, проти якої написаний CLAUDE.md §0 п.3**. Тому
  класифікатор будується як «дві відомі сигнатури + все інше», а не як три рівноправні
  гілки: невідоме падіння за замовчуванням іде в `failed`, а не в «мабуть, квота».
- _Наслідки:_ (+) найдорожчий історичний клас відмов (401, тиждень простою) ловиться
  першим же матчем. (−) до першого реального вичерпання квоти агент трактуватиме його як
  `failed` і зупинить чергу після трьох поспіль — прийнятна поведінка (безпечна сторона).
  **Обов'язок:** коли квотне падіння станеться вперше — дослівний текст із журналу
  заноситься сюди, у цю таблицю, тим самим механізмом, яким сюди потрапив 401.

**5. Два квотні пули: Claude — write, `agy`/Gemini — read-only.**

- _Альтернативи:_ один пул (Claude на все) — упирається у вікно квоти; рівноправні пули
  (Gemini теж пише) — відхилено даними.
- _Вибір:_ **Claude** виконує всі write-роботи (код, PR, правки доків). **`agy`/Gemini**
  — лише read-only: тріаж, незалежне рев'ю, інвентаризації; і той самий read-only
  резерв, коли Claude-вікно вичерпане. Вихід `agy` пишеться **тільки** у
  `docs/promts/inputs/**` і **завжди** перевіряється Claude або yurii перед тим, як
  стати підставою для дії.
- _Обґрунтування (кількісне):_ `agy-stats.md` — 3/3 виклики з ≥1 вигаданим фактом
  (2, 1, 1). Три точки — мало для статистики, але напрямок консистентний і збігається з
  2 інцидентами самовільного запису (Master Run 8, `docs/17` §9). Модель, яка
  систематично додає неіснуючі деталі, придатна як **джерело гіпотез** і непридатна як
  джерело фактів без верифікації. Водночас саме незалежність вендора робить її цінною в
  рев'ю (ADR-036 §3: «розбіжність двох моделей цінніша за їхню згоду»).
- _Передумова, ще не виконана:_ спосіб логіна `agy` на headless-машині НЕ ПЕРЕВІРЕНО
  (`docs/19` §D.4, три варіанти). Доки D.4 не закритий, Gemini-пулу на A8 немає, і
  резерв при вичерпаній Claude-квоті = пауза (складова 4).
- _Наслідки:_ (+) дешевий пул закриває найдорожчий за обсягом клас робіт (читання).
  (−) кожен `agy`-вихід потребує верифікатора → економія менша за номінальну.

**6. Протокол питань до yurii — два класи, обов'язкові поля, бюджет.**

- _Клас A (незворотне або зміна контракту):_ **дефолту немає**, агент чекає скільки
  треба. Канал `#питання-блокуючі`. Сюди належить усе з переліку виключень ADR-040 §2 і
  все, що змінює CLAUDE.md §0/§2/§7 або ADR.
- _Клас B (оборотне):_ є ★рекомендація і дефолт, **таймер 6–12 год** (Q7); мовчання =
  застосування дефолту, факт застосування записується в журнал і в добовий дайджест.
  Канал `#питання-дефолт`.
- _Обов'язкові поля будь-якого питання (шість):_ **питання / варіанти / наслідок
  кожного варіанта / ★рекомендація / дефолт-при-мовчанні / оборотність**. Питання, у
  якому хоч одне поле порожнє, **не відправляється** — супервізор відхиляє його як
  невалідне і повертає агенту. Це механічна перевірка, не побажання.
- _Бюджет:_ **не більше 3 питань на задачу і 10 на добу** сумарно. Перевищення = задача
  зупиняється зі звітом «потрібне уточнення постановки», бо потік дрібних питань шкодить
  так само, як вигадки (CLAUDE.md §0 п.4).
- _Канали Discord (чотири, `docs/19` §D.6, заводяться через `infra/discord/` IaC,
  ADR-023):_ `#питання-блокуючі`, `#питання-дефолт`, `#на-перевірку` (draft PR +
  посилання на прев'ю + «Як перевірити очима»), `#інциденти` (401, квота, машина впала,
  диск).
- _Наслідки:_ (+) yurii отримує рішення, а не задачі-на-подумати; клас B не блокує
  прогрес. (−) дефолт може виявитись неправильним — саме тому клас B обмежений
  оборотним, і саме тому «скільки дефолтів yurii потім скасував» — метрика (складова 8).

**7. Запобіжники.**

- **Kill switch** — файл-прапорець на диску A8; супервізор перевіряє його ПЕРЕД кожною
  задачею і зупиняє чергу без завершення поточного PR. Має існувати **до першого
  автономного прогону**, не після (`docs/19` §7 п.5).
- **Денний ліміт задач** (Q9) — конкретне число задає A8-run; перевищення = пауза до
  наступної доби.
- **Журнал кожного прогону на диску**: задача, гілка, worktree, модель, час, клас
  виходу, посилання на PR. Журнал — не лог агента, а структурований запис супервізора
  (агент не пише сам про себе).
- **Три `failed` поспіль → пауза** (складова 4).

**8. Метрики самовдосконалення** (без них ADR-039 — набір намірів):

| Метрика                         | Як рахується                                                                                                        | Навіщо                                                                                                               |
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| **Покриття оракулами**          | частка задач, приймання яких доведене ПРОГРАМОЮ (тест/скрипт/снапшот), а не твердженням агента                      | пряма реакція на §A інспекції: «зелений тест» ≠ «інваріант enforced». Ціль — щоб частка росла, а не щоб була високою |
| **Якість питань**               | скільки питань класу A yurii позначив як «можна було дефолтом» + скільки дефолтів класу B він потім скасував        | ловить обидва перекоси: агент питає забагато / агент вирішує сам там, де не мав                                      |
| **`agy-stats`**                 | наявний журнал (`docs/promts/inputs/agy-stats.md`), колонка «вигаданих фактів» на виклик                            | кількісна підстава переглядати роль Gemini-пулу (складова 5)                                                         |
| **Правило трьох (пост-мортем)** | клас помилки, що повторився **тричі**, АВТОМАТИЧНО породжує задачу «змінити процес» (ADR / гейт / оракул / правило) | переводить CLAUDE.md §0 п.6 з обіцянки в механізм: третій повтор створює задачу без участі людини                    |

**Наслідки (загальні):**

- `docs/16_AUTONOMOUS_RUNS.md` §1 приводиться до правди цим самим PR (інспекція §C.2).
- Портативні deny-правила у **трекованому** `.claude/settings.json` — обов'язковий
  пункт A8-run'у (`docs/19` §6): без них інваріант «правило, якого немає в git, не
  захищає ніщо» (`docs/19` §7 п.4) лишається порушеним.
- Нові оракули, яких сьогодні немає і які випливають з §A інспекції (кандидати в беклог
  генерального плану, не в цей ADR): import-usage checker (валідатор без виклику в
  пайплайні = червоний CI); guard «`baked-spec.ts` у git = свіжій генерації»;
  test-first proof job (закриває ручний крок 3-bis рев'юера, `docs/17` §9).
- Digest-cron (інспекція §D, §E): механізму планування немає в git узагалі. A8-run
  зобов'язаний або внести його в Ansible/systemd, або зняти «✅» у `docs/11` §11.
- `AGENTS.md` отримує секцію «Середовище автономної розробки (A8)» — цей PR.

**Альтернативи (загальні):** Actions-джоба як двигун агентів (Q6) — відхилено:
6-годинний ліміт job'а, немає стану між прогонами, кожен запуск платний, і саме в цьому
режимі стались 5 падінь тріажу. Хмарний агент-сервіс — відхилено: prod-креденшали й
код у чужому середовищі, ADR-011-логіка (дані ближче до автора) і вартість. Людина в
контурі на кожен крок — це статус-кво, який run і скасовує; він не масштабується
далі одного PR за сесію.

---

## ADR-040: Auto-merge оборотного класу у тестовому режимі

**Статус:** Accepted (2026-08-11, Master Run 14 Стадія 2). Діє **лише поки платформа в
тестовому режимі**; тригер скасування — публічний launch (див. §4).

**Контекст:** Рішення Q11 (`docs/19` §0.2). ADR-036 §5 фіксує «merge будь-якого PR —
лише yurii», і це працювало, поки одиницею роботи був один PR за сесію. Автономне
середовище (ADR-039) генерує PR швидше, ніж yurii встигає їх читати; черга непрочитаних
draft-PR — це той самий простій, лише перекладений на іншу поверхню. Водночас інспекція
§A показує, що «всі оракули зелені» — слабший сигнал, ніж здається: три інваріанти зі
статусом `декларація` пройдуть повний CI зеленим вічно.

**Рішення:**

**1. Оборотний клас визначається ВИКЛЮЧЕННЯМИ, не переліком дозволеного.** Перелік
дозволеного старіє мовчки (новий каталог за замовчуванням стає дозволеним); перелік
виключень старіє голосно. **PR НЕ належить до оборотного класу, якщо його diff
(`git diff --name-only origin/main...HEAD`) зачіпає хоч один із шляхів:**

- `packages/db/src/migrations/**` — міграції БД (незворотні за визначенням);
- `infra/**` — інфраструктура і деплой;
- `.github/workflows/**` — самі оракули (агент не змінює правила, за якими його
  перевіряють);
- `CLAUDE.md` — контракт виконавця;
- `docs/03_DECISIONS.md` — ADR;
- `docs/12_TEMPLATE_CONTRACT.md` — контракт Template Registry;
- `packages/db/src/schema.ts` — схема БД;
- `packages/cad-engine/data/bend-machine-esi.yaml` — bend-матриця, single source of
  truth (CLAUDE.md §13);
- `workers/cad/tests/snapshots/**` — **будь-яка зміна byte/структурних снапшотів
  DXF/PDF**: зміна снапшота означає зміну того, що приймає замовник на виробництві.

**Механізм перевірки вже існує** — `tools/scripts/check-forbidden-paths.sh:19-26`
(8 regex) + `ai-fix.yml:62-81` (19 записів `deny` = 8 файлових цілей парою `Edit`+`Write` + 3 Bash-патерни): його треба перевикористати, а не
писати заново. Звірка списків (перевірено 2026-08-11, не на око):

- **8 із 9 позицій уже покриті** наявними патернами;
- **`packages/cad-engine/data/bend-machine-esi.yaml` у наявному скрипті ВІДСУТНІЙ** —
  його треба **додати** при реалізації auto-merge, інакше bend-матриця опиниться в
  оборотному класі, чого цей ADR не допускає;
- наявний патерн `^\.github/` **ширший** за `.github/workflows/**` — розбіжність на
  користь безпеки, звужувати не треба;
- `docs/12_TEMPLATE_CONTRACT.md` і `packages/db/src/schema.ts` є у скрипті, але їх не
  було у формулюванні промпту Master Run 14 — додані сюди свідомо: контракт реєстру і
  схема БД оборотними не є.

**2. Умови auto-merge — усі три одночасно:**

1. **Усі оракули зелені** — 10 job'ів `ci.yml` без винятків (жодного `continue-on-error`,
   жодного пропущеного job'а);
2. **Вердикт незалежного рев'юера ✅** (ADR-036 §3; рев'юер — не та сесія, що писала код);
3. **Diff поза переліком виключень §1** — перевірка скриптом, не оком.

Невиконання будь-якої умови → звичайний draft PR у `#на-перевірку`, гейт yurii.

**3. Після merge — smoke і авто-відкат:**

deploy на staging → smoke-тест → **червоний smoke = автоматичний `git revert` merge-коміту
і повідомлення в `#інциденти`**. Відкат робиться без питання до yurii: непрацюючий
staging дорожчий за зайвий revert. Добовий **дайджест змердженого** (перелік PR, що
пройшли auto-merge за добу, з посиланнями) — у `#на-перевірку`.

**4. Тригер скасування — умова, не намір.**

Auto-merge **автоматично припиняє діяти в момент публічного launch** (трек T3
`docs/02_ROADMAP.md`, go-live PR за `docs/08` §8). Не «переглянемо перед launch», не
«вирішимо тоді» — прапорець auto-merge вимикається тим самим PR, який робить платформу
публічною, і повернути його можна лише новим ADR. Причина: до launch ціна помилки —
зіпсований staging; після launch — зіпсоване креслення в чужому виробництві (R-01).

**Наслідки:**

- (+) Автономний контур замикається: агент може довести оборотну задачу до `main` без
  людини, і саме це робить середовище ADR-039 корисним, а не просто швидшим генератором
  чернеток.
- (−) **Зелений CI — слабший доказ, ніж виглядає** (інспекція §A: `validateSheet`,
  `validateHoles`, `isValid`). Auto-merge підвищує ціну цього розриву, тому
  import-usage checker (ADR-039, «нові оракули») стає пріоритетним пунктом беклогу, а
  не «колись потім».
- (−) Помилка потрапляє в `main` без читання людиною. Компенсації три: виключення (§1),
  auto-revert (§3), добовий дайджест (§3).
- (=) ADR-036 §5 не скасовується, а **звужується**: «merge — лише yurii» лишається
  чинним для всього, що не є оборотним класом у тестовому режимі. Після launch §5
  повертається в повному обсязі автоматично.

**Альтернативи:** merge усього зеленого (без виключень) — відхилено: міграції й
снапшоти незворотні, а їх «зеленість» нічого не доводить. Лишити статус-кво (усе через
yurii) — відхилено як причина простою, яку і має зняти ADR-039. Auto-merge з
відкладеним вікном (merge через 24 год, якщо yurii не заперечив) — відхилено: створює
ілюзію перевірки й ту саму чергу непрочитаного, лише з таймером.

---

## ADR-041: Критерії переїзду prod з Mirohost MS21 — метрики і пороги (рішення НЕ приймається)

**Статус:** **Proposed** (2026-08-11, Master Run 14 Стадія 2). **Чинним лишається
ADR-011** (Mirohost Cloud MS21). Цей запис не переносить нічого і нікуди — він фіксує
наперед, ЩО вимірюємо і ЯКІ числа зроблять переїзд допустимим.

**Контекст:** Рішення Q4 (`docs/19` §0.2): поріг фіксується числом **до** збору даних —
інакше після кварталу експлуатації A8 будь-який результат виглядатиме як підтвердження
того, у що вже віриш. Трек T5 п.2 (`docs/02_ROADMAP.md`) ставить питання «міні-ПК /
переїзд prod» у залежність від uptime-даних, яких сьогодні немає жодних.

Що вже відомо і не потребує вимірювання: R-11 (`docs/04_RISKS.md:218`) — увесь стек на
одному вузлі, HA немає, RPO до 24 год; ADR-011 обрав MS21 за співвідношенням ціна/
надійність для 10 користувачів/день.

**Рішення: вимірювати A8 протягом ОДНОГО кварталу з дня, коли машина стала робочою
(Checkpoint C зелений, `docs/19` §4), і зафіксувати такі метрики й пороги.**

| #   | Метрика                                                   | Як вимірюється                                                                  | Поріг, за якого переїзд prod на A8 стає ДОПУСТИМИМ       |
| --- | --------------------------------------------------------- | ------------------------------------------------------------------------------- | -------------------------------------------------------- |
| 1   | **Uptime, %** за квартал                                  | healthcheck-cron A8 → `#інциденти` (`docs/19` §6), агрегація по хвилинах        | **≥ 99.5 %** (≤ ~3.3 год недоступності за 90 днів)       |
| 2   | **Кількість відключень живлення**                         | лічильник подій «машина завантажилась без graceful shutdown» (`last -x reboot`) | **≤ 2 за квартал**                                       |
| 3   | **Сумарна тривалість відключень живлення й мережі**       | той самий журнал + фіксація втрати WAN                                          | **≤ 2 год за квартал сумарно**                           |
| 4   | **Час автовідновлення після ребуту** (до зеленого health) | від моменту старту BIOS до відповіді healthcheck                                | **≤ 5 хв, і 100 % ребутів завершились автопідйомом**     |
| 5   | **Throttle під навантаженням**                            | температура/частота CPU під час повного CI-прогону (`sensors`, `cpufreq`)       | **0 подій throttling** протягом кварталу                 |
| 6   | **Вільний диск**                                          | `df -h /` щодня                                                                 | **≥ 30 % вільного** на кінець кварталу, тренд не спадний |

**Переїзд допустимий лише при виконанні ВСІХ шести порогів.** Невиконання будь-якого —
prod лишається на MS21, а квартал вимірювання починається заново після усунення причини.
«Допустимий» ≠ «відбудеться»: це необхідна умова, рішення ухвалюється окремим ADR.

**Наслідки, які НЕ є технічними** (їх не закриє жодна метрика — тому вони перелічені
тут, а не «потім»):

1. **Фізичне розташування даних у Privacy Policy.** ADR-011 уже містить пункт про
   розташування даних; переїзд на домашню машину змінює його на «приватне помешкання у
   <країна>». `/privacy` + `/privacy/en` доведеться оновити, і це заява про обробку
   персональних даних — тобто крок, який потребує юриста (трек T3 п.4), а не правки
   тексту агентом.
2. **GDPR-формулювання.** Домашній хостинг не заборонений, але вимоги до захисту
   (фізичний доступ, шифрування диска, реакція на інцидент) формально ті самі, що до
   дата-центру. Ансибл-роль і UFW їх не закривають — потрібне рішення про шифрування
   диска A8 і про процедуру на випадок фізичного вилучення/крадіжки машини.
3. **R-11 змінює характер, а не величину.** На MS21 ризик — «збій хоста провайдера»
   (компенсується runbook'ом ~30 хв, `docs/08` §5.5). На A8 додається клас, якого в
   MS21 не було: відключення світла, аварія провайдера в квартирі, фізичний доступ
   третіх осіб, від'їзд yurii. Це не «менш надійно», це **інший профіль ризику**, і
   порівнювати їх одним числом uptime некоректно — саме тому метрик шість, а не одна.
4. **Війна.** Це не абстракція для цього проєкту: фізичне розташування єдиного вузла в
   Україні — параметр, який не вимірюється uptime'ом за квартал.

**Що цей ADR НЕ робить:** не приймає рішення про переїзд; не описує план міграції; не
скасовує ADR-011; не змінює нічого в `infra/`. План міграції пишеться тоді (і тільки
тоді), коли всі шість порогів пройдено — інакше це сторінки тексту про подію, якої може
не бути.

**Альтернативи:** приймати рішення зараз, за інтуїцією — відхилено (CLAUDE.md §0 п.3:
здогад, поданий як факт). Писати повний план міграції одразу разом з метриками —
відхилено (Опитування п.3 цього run'у, ★перший варіант): план, написаний за квартал до
рішення, застаріє швидше, ніж знадобиться. Не вимірювати взагалі й лишити T5 п.2
відкритим — відхилено: питання не зникає, а без метрик воно вирішується настроєм.
