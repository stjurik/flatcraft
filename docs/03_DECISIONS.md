...
**Примітка (закрита гілка процесу, Master Run 15, 2026-08-12).** Рішення 2 цього
ADR назвало три термінальні лейбли тріажу, але для `ai-architectural` не описало,
що відбувається ПІСЛЯ. Наслідок виявився на issue **#96**: тріаж відпрацював за
1 хв 50 с і класифікував коректно, yurii відповів по суті того ж дня — і задача
простояла 10 діб, бо коментар тріажу не містив рішення-запиту, а відповіді yurii
не було куди піти. Гілка закрита: `docs/17` §3-bis (шість обов'язкових блоків
коментаря, крок «відповідь → черга треку», зняття лейбла разом із відповіддю) +
відповідна вимога у тексті промпту `ai-triage.yml`. **Рішення 1-5 цього ADR не
переглядаються** — правка стосується лише того, чого в них не було сказано.
Механічної перевірки повноти коментаря немає: крок verify перевіряє лейбл, не
зміст (обмеження зафіксовано у `docs/17` §3-bis і в #78).
