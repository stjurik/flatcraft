...
- _Обґрунтування (кількісне, уточнено 2026-09-14):_ `agy-stats.md` — **4 виклики, з них
  3 з ≥1 вигаданим фактом (2, 1, 1, 0)**. Четвертий (перехресний скан Master Run 14,
  2026-08-10) дав 0 вигадок на вибірці, заздалегідь звуженій до перевірюваних фактів —
  висновку він не змінює. Чотири точки — мало для статистики, але напрямок консистентний і збігається з
...
- _Передумова, виконана частково (оновлено 2026-09-14, PR #108):_ **механіка**
  headless-виклику `agy` перевірена зондом `tools/scripts/check-agy-headless.sh` — на T470
  exit 0, ✅ PASS (`docs/19` §D.4, таблиця вердиктів 0-8 замінила три розпливчасті варіанти).
  **Логін на A8 досі відкритий**: креденшали на T470 отримані через браузер, а A8 headless.
  Той самий блокер стоїть перед D.3 (`claude setup-token`) — обидва закриває
  `docs/promts/a8-stage-0-login.md`. Доки логін не закритий, Gemini-пулу на A8 немає, і
