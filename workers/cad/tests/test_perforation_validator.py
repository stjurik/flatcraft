...
Включає property-based parity проти незалежного oracle — дзеркало
...
class TestPerforationUnit:
    def test_square_clean_grid_valid(self) -> None:
        errs = validate_perforation(
            ...
            {"hole_size_mm": 8, "pitch_x_mm": 30, "pitch_y_mm": 30},
        )
        assert errs == []
    ...
    def test_square_pitch_y_below_side_overlaps(self) -> None:
        errs = validate_perforation(
            "perforated_panel_square",
            {"hole_size_mm": 20, "pitch_x_mm": 27, "pitch_y_mm": 10},
        )
        assert len(errs) == 1
    ...
    def test_square_touching_zero_ligament_invalid(self) -> None:
        ...
        errs = validate_perforation(
            "perforated_panel_square",
            {"hole_size_mm": 30, "pitch_x_mm": 30, "pitch_y_mm": 30},
        )
    ...
    def test_round_below_diameter_overlaps(self) -> None:
        errs = validate_perforation(
            "perforated_panel",
            {"hole_diameter_mm": 25, "pitch_x_mm": 12, "pitch_y_mm": 12},
        )
        assert len(errs) == 2
...
def test_property_parity_with_oracle(hole: float, px: float, py: float, square: bool) -> None:
    slug = "perforated_panel_square" if square else "perforated_panel"
    hole_key = "hole_size_mm" if square else "hole_diameter_mm"
    errs = validate_perforation(slug, {hole_key: hole, "pitch_x_mm": px, "pitch_y_mm": py})
