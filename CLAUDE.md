...
- **Phase 2.4 завершено** (2026-05-17): `packages/ui/parameter-form/` — pure `introspectSchema(zodObject) → FieldDescriptor[]` (13 unit) + AutoForm React-компонент. L-bracket editor мігровано з ручних inputs на AutoForm; testIds збереглися, e2e зелені. Holes (array) поки не підтримується generic'ом — рендериться окремим renderField override (повноцінний holes-editor у Phase 2.5).
- **Наступне — Phase 2.5**: live-валідація з підсвіченням обмежень (червоні поля + tooltip з причиною).
