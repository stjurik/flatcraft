...
- **Phase 5 INFRASTRUCTURE (паралельний трек, in progress, 2026-05-18..22):**
  ...
  - **Не зроблено ще:** production deploy (Roadmap 5.10); R-11 у `docs/04_RISKS.md` (так і не дописано — ризики йдуть R-09, R-10); звузити `ssh_allowed_ips` з `0.0.0.0/0` у `all.yml` перед production; узгодити `deploy_authorized_keys` у `all.yml` (`mirohost-cloud`) з фактичним ключем на сервері (важливо лише при `--tags provision`).
...
_Останнє оновлення: 2026-05-28. Коли архітектура змінюється — оновлюйте цей файл першим, потім код._
