...
2. **Реалізацію перевіряє не автор.** ШІ-код рев'юїть інший агент (ADR-036
   §3). Шляхів два: основний — через `agy`, як описано нижче; окрема сесія
   Claude (не та, що писала код) теж допустима, зокрема для архітектурних і
   безпекових PR, — і тоді в PR прямо пишеться, що рецензію зроблено цим
   шляхом. Вердикт публікується ДОСЛІВНО.
   ...
   _Рецензія через `agy` (рішення yurii 2026-09-24):_ дві моделі — основна
   **Gemini 3.8 Flash (High)**, модель не з родини Claude, і друга, коротка,
   лише контрприклади, **Claude Opus 4.6**. Їхній вихід — джерело гіпотез,
   яке верифікує Claude або yurii, а не джерело фактів; незгода рецензентів
   показується в PR обома позиціями дослівно. Роль не безумовна: **кожна
   модель складає свій іспит — вердикт лише за підсумками її перших 10
   викликів від 2026-09-15**; якщо частка викликів з ≥1 вигаданим фактом не
   нижче 50%, модель іде з ролі Рев'юера. Поріг 10 викликів / 50%
   зареєстровано 2026-09-15 для Gemini (`docs/20` OQ-35), 2026-09-24 yurii
   поширив його на кожну модель окремо. Так 2026-09-23 з ролі Рев'юера пішов
   **Gemini 3.1 Pro** (10 викликів, 5 з вигадками) — за правилом 2026-09-15 у
   Розвідника. Журнал `docs/promts/inputs/agy-stats.md` рахує для кожної
   моделі і вигадки, і **справжні знахідки** — твердження, що пройшли
   перевірку й привели до правки; числа рахує
   `tools/scripts/agy-stats-summary.sh`, тут їх не пишемо — записане число
   застаріває. Кожен виклик логується в `agy-stats.md` — незалогований робить
   поріг недійсним. Власні дозволи `agy`: читання лише репо й worktree-ів,
   запис лише в `docs/promts/inputs/`, жодної команди (`tools/agy/`; доказ —
   `tools/scripts/agy-permissions-probe.sh`).
...
5. **Незалежне рев'ю** — інший агент, не той, що писав код (ADR-036 §3).
   ...
   розбіжність двох моделей цінніша за їхню згоду. Два шляхи рецензії, моделі
   `agy` та іспит кожної з них — §0 п.2.
