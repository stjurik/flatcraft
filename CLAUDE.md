...
**Де ми зараз (2026-06-26):** staging.hart.crimea.ua live, MVP feature-complete. Серверна валідація радіусу гибу — інваріант (ADR-019); render-gate проти крашу R3F (ADR-026). DXF production-grade для CAM (2 шари, ADR-024); PDF з довідковою ізометрією (ADR-025). Перфо-панель **уніфіковано в ОДИН шаблон** `perforated_panel` (ребристий монтажний лоток), де форма отвору — параметр `hole_shape` circle|square (ADR-031, supersedes ADR-029, розширює ADR-030); ребра вниз. Discord-сервер як IaC (ADR-023) чекає manual-setup. Наступний фокус — публічний soft-launch.
...
_Останнє оновлення: 2026-06-26. Коли архітектура змінюється — оновлюйте CLAUDE.md (§1-12) першим. Завершення фази — у `docs/13_PROGRESS_LOG.md` + ротація §13._
