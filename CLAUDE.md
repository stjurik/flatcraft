...
**Де ми зараз (2026-07-06):** staging.hart.crimea.ua live, MVP feature-complete. **Phase 3.3 (observability foundation, ADR-032)** — перед soft-launch платформа отримала телеметрію: Postgres `events` (append-only) + persist `exports`, Sentry ×3 з `beforeSend` PII-фільтром, щотижневий digest → Discord, воронка **Umami self-hosted** + web-vitals. GDPR — без PII (session*hash з добовим salt). **Активація телеметрії в проді — окремо:** env (`SENTRY_DSN`, `NEXT_PUBLIC_UMAMI*\*`, `DIGEST_WEBHOOK_URL`) + деплой Umami-контейнера (compose/Ansible PR, ще не зроблено). Перфо-панель — ОДИН шаблон `perforated_panel` (`hole_shape`, ADR-031). Render-gate проти крашу R3F (ADR-026); DXF production-grade (2 шари, ADR-024); PDF з ізометрією (ADR-025). Discord IaC (ADR-023) чекає manual-setup. Наступний фокус — публічний soft-launch.

**Останні 3 milestones:**

- **Feature 3.3 — Observability foundation** (2026-07-06, ADR-032): 6 PR перед soft-launch. (1) docs-gate ADR-032 + docs/11; (2) Postgres `events` (Zod-payload'и) + persist `exports` (nullable user/draft, session*hash з добовим salt); (3) Sentry ×3 web/api/worker з `beforeSend` PII; (4) pure `buildDigest` → Discord webhook (cron нд 18:00); (5) воронка **Umami self-hosted** (vendor-нейтральний `track`, GDPR fail-closed) + web-vitals; (6) цей progress-log. Стек мінімальний (без Prometheus/Grafana/OTel). Аналітика — Umami (окрема БД у наявному Postgres; Plausible відхилено — ClickHouse RAM). PR #54-#58. Міграцію events/exports генерує yurii вручну. **Активація в проді — окремо:** env (`SENTRY_DSN`, `NEXT_PUBLIC_UMAMI*\*`, `DIGEST_WEBHOOK_URL`) + деплой Umami-контейнера (compose/Ansible PR, ще не зроблено).
...
_Останнє оновлення: 2026-07-06. Коли архітектура змінюється — оновлюйте CLAUDE.md (§1-12) першим. Завершення фази — у `docs/13_PROGRESS_LOG.md` + ротація §13._
